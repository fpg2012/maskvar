...
class DummyTester(nn.Module):
    ...
    def __init__(self, twt: AdaptedTwoWayTransformer, max_len=100, vocab_size=1024, embed_dim=256, device='cuda'):
        super().__init__()
        self.twt = twt
        self.cls = nn.Linear(twt.embedding_dim, 1024)
...
if __name__ == '__main__':
    ...
    model = DummyTester(twt, max_len=max_len, vocab_size=vocab_size, embed_dim=embed_dim, device=device)
    model.to(device)
