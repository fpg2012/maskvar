...
class FlexMaskVARSimple(nn.Module):
    """
    ...
    """
    def __init__(
        self, vae_local: VQVAE, prompt_encoder: PromptEncoder,
        ...
        # )
        print(
            f'\n[constructor]  ==== (fusing_add_ln={sum(fused_add_norm_fns)}/{self.depth} ==== \n'
            f'    [VAR config ] embed_dim={embed_dim}, num_heads={num_heads}, depth={depth}, mlp_ratio={mlp_ratio}\n'
            f'    [drop ratios ] drop_rate={drop_rate}, drop_path_rate={drop_path_rate:g} ({torch.linspace(0, drop_path_rate, depth)})',
            end='\n\n', flush=True
        )
    ...
    def autoregressive_infer_cfg(
        ...
        image_feats = F.interpolate(sam_image_embedding, size=(self.patch_nums[-1], self.patch_nums[-1]), mode='bilinear', align_corners=False) # B C H W
        image_pe = self.prompt_encoder.get_dense_pe()
        image_feats = image_feats + image_pe
        image_feats = rearrange(image_feats, 'B C H W -> B (H W) C')
            ...
            with record_function(f'autogressive pass {si}'):
                ...
                for b in self.blocks:
                    with record_function(f'autogressive pass {si} block {b}'):
                        ...
                        x = b(x=x, cond=image_feats, prompt_cond=prompt_embedding, block_mask=None)
                ...
                with record_function(f'autogressive pass {si} get next input'):
                    h_BChw = rearrange(h_BChw, 'B (pn pn) C -> B C pn pn', pn=pn)
    ...
    def forward(self, 
        ...
        image_feats = F.interpolate(sam_image_embedding, size=(self.patch_nums[-1], self.patch_nums[-1]), mode='bilinear', align_corners=False)
        ...
        image_feats = image_feats.to(dtype=main_type)

        # add image_pe to image_feats
        image_pe = self.prompt_encoder.get_dense_pe()
        image_feats = image_feats + image_pe
        image_feats = rearrange(image_feats, 'B C H W -> B (H W) C')
        ...
        for i, b in enumerate(self.blocks):
            with record_function(f"transformer_block_{i}"):
                ...
                if self.attention_checkpoint:
                    ...
                    x_BLC = b(x=x_BLC, cond=image_feats, prompt_cond=prompt_embedding, block_mask=self.block_mask)
