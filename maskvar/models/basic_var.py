...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, cond, attn_bias=None):
        ...
        q = F.linear(input=x, weight=self.mat_q.weight, bias=self.q_bias).view(B, -1, 1, self.num_heads, self.head_dim)
        # print(f'CrossAttention: q.shape: {q.shape}')
    
        k = F.linear(input=cond, weight=self.mat_k.weight, bias=self.zero_k_bias).view(B, -1, 1, self.num_heads, self.head_dim)
        v = F.linear(input=cond, weight=self.mat_v.weight, bias=self.v_bias).view(B, -1, 1, self.num_heads, self.head_dim)
        # print(f'CrossAttention: k.shape: {k.shape}, v.shape: {v.shape}')
...
class CrossAttn(nn.Module):
    def __init__(self, block_idx, last_drop_p, embed_dim, cond_dim, shared_aln: bool, norm_layer,
        ...
        self.C, self.D = embed_dim, cond_dim
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.self_attn = SelfAttention(block_idx=block_idx, embed_dim=embed_dim, num_heads=num_heads, attn_drop=attn_drop, proj_drop=drop, attn_l2_norm=attn_l2_norm, flash_if_available=flash_if_available)
        ...
        self.prompt_attn = CrossAttention(block_idx=block_idx, embed_dim=embed_dim, num_heads=num_heads, attn_drop=attn_drop, proj_drop=drop, attn_l2_norm=attn_l2_norm, flash_if_available=flash_if_available)
        self.ffn = FFN(in_features=embed_dim, hidden_features=round(embed_dim * mlp_ratio), drop=drop, fused_if_available=fused_if_available)
        self.ffn2 = FFN(in_features=embed_dim, hidden_features=round(embed_dim * mlp_ratio), drop=drop, fused_if_available=fused_if_available)
        self.ffn3 = FFN(in_features=embed_dim, hidden_features=round(embed_dim * mlp_ratio), drop=drop, fused_if_available=fused_if_available)
