...
class SimpleMaskMaskGITTrainer:
    ...
    def __init__(
        ...
        self.cfg_drop_click_prob = cfg_drop_click_prob if enable_cfg else 0.0
        self.cfg_drop_image_prob = cfg_drop_image_prob if enable_cfg else 0.0
            ...
            for batch in self.train_loader:
                ...
                if self.enable_click:
                    click_coords, click_labels = sample_click_conditions(
                        single_mask,
                        self._get_maskgit_model().h,
                        self._get_maskgit_model().w,
                        max_clicks=2,
                    )
                else:
                    click_coords = None
...
def main():
    ...
    parser.add_argument('--cfg_drop_click_prob', type=float, default=0.1,
                        help='Per-sample probability of dropping click condition during CFG training')
    parser.add_argument('--cfg_drop_image_prob', type=float, default=0.0,
                        help='Per-sample probability of dropping image tokens during CFG training')
    parser.add_argument('--cfg_guidance_scale', type=float, default=1.0,
                        help='CFG scale used for optional pure inference IoU')
    ...
    if rank == 0:
        ...
        if args.enable_cfg:
            print(
                "CFG: enabled "
                f"(drop_click_prob={args.cfg_drop_click_prob}, "
                f"drop_image_prob={args.cfg_drop_image_prob}, "
                f"guidance_scale={args.cfg_guidance_scale}, "
    ...
    trainer = SimpleMaskMaskGITTrainer(
        ...
        cfg_drop_click_prob=args.cfg_drop_click_prob,
        cfg_drop_image_prob=args.cfg_drop_image_prob,
