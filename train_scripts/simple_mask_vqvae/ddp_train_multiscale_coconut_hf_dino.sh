#!/bin/bash
# Train multi-scale SimpleMaskVqvae on COCONut HF dataset
# Decoder is the existing SimpleMaskDecoder; tokenizer uses scales 1..64.

N_NODE=${1:-4}
OUTDIR=${2:-out/ddp_simple_mask_vqvae_multiscale_coconut_ep10}
...
if [ $N_NODE -eq 0 ]; then
    ...
    --prefetch_factor 4 \
    --dataset coconut_hf \
    --config simple_mask_vqvae_multiscale_dim384 \
